...
def _get_pixels(per_pixel, rand_color, patch_size, dtype=tf.float32, device="gpu"):
    if per_pixel:
        return tf.random.normal(patch_size, dtype=dtype)
    elif rand_color:
        return tf.random.normal((patch_size[0], 1, 1), dtype=dtype)
    else:
        return tf.zeros((patch_size[0], 1, 1), dtype=dtype)
        ...
        for _ in range(count):
            for _ in range(10):
                ...
                if w < img_w and h < img_h:
                    ...
                    img[:, top : top + h, left : left + w].assign(_get_pixels(
                        self.per_pixel,
                        self.rand_color,
                        (chan, h, w),
                        dtype=dtype,
            ...
            for _ in range(100):
                ...
                if w < img_w and h < img_h:
                    ...
                    for i in range(batch_start, batch_size):
                        img_instance = img[i]
                        img_instance[:, top : top + h, left : left + w].assign(_get_pixels(
                            self.per_pixel,
                            self.rand_color,
                            (chan, h, w),
                            dtype=dtype,
