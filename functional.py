...
def _is_tensor_clip(clip):
    return tf.is_tensor(clip) and len(clip.shape) == 4
...
def normalize(clip, mean, std, inplace=False):
    if not _is_tensor_clip(clip):
        raise TypeError('tensor is not a tensorflow clip.')

    if not inplace:
        clip = tf.identity(clip)

    mean = tf.convert_to_tensor(mean, dtype=clip.dtype)
    std = tf.convert_to_tensor(std, dtype=clip.dtype)
